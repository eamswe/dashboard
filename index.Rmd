...
spread_hosp <-  spread_hosp %>% 
  select(date, Ireland) %>% 
  rename(hosp_rate = Ireland)

JHU_IE <- merge(spread_hosp, JHU_covid, by="date")


JHU_IE <- JHU_IE %>% 
  rename(
    "Hospitalised Covid-19 cases per 100,000" = hosp_rate,
    "14 day incidence rate per 100,000" = incidence_14day_per_100k
  )

IE_xts <- tk_xts(JHU_IE, select = `Hospitalised Covid-19 cases per 100,000` |  `14 day incidence rate per 100,000`, date_var = date) 
dygraph(IE_xts) %>%
  dyOptions(colors = RColorBrewer::brewer.pal(8, "Dark2")) %>%
  dyAxis("y", label = "Cases")%>%
  dyAxis("y2", label = "Hospitalised cases ") %>%
  dySeries("Hospitalised Covid-19 cases per 100,000", axis=('y2'))
...
Row 
-----------------------------------------
### Is this dashboard being updated?
